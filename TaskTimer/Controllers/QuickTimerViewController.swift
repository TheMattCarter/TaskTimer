...
class QuickTimerViewController: UIViewController {
   ...
   var player: AVAudioPlayer!
    ...
    func setDefaultQuickTimerPrefs() {
        timerCurrentTime =
        Int(defaults.object(forKey: "defaultCountDownTimeDuration") as! TimeInterval)
        workTime = Int(defaults.object(forKey: "defaultWorkTimeDuration") as! TimeInterval)
        breakTime = Int(defaults.object(forKey: "defaultBreakTimeDuration") as! TimeInterval)
        qtyIntervals = defaults.integer(forKey: "defaultQtyIntervals")
        
        //converting timer mode to int
               let defaultTimerMode = defaults.string(forKey: "defaultQuickTimerMode")
               
               //"defaultQuickTimerMode" = ["Countdown", "Interval", "Running Time"]
               switch defaultTimerMode {
               case "Countdown":
                   timerMode = 2;
               case "Interval":
                   timerMode = 1;
               case "Running Time":
                   timerMode = 0;
               default:
                   print("error selecting quicktimer mode")
               }
    }
